...
Aeris 22.0.0-alpha.8 is alpha software for Angular >=22.0.6 <23.0.0. It is not production ready.
...
| `showIcon` | `boolean` | `true` | Shows the default or projected icon for each message. |
...
| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `closed` | `AerisToastCloseEvent` | `-` | Emits when a message rendered by this Toast closes by timeout, button, swipe, API, or clear. |
...
| `--aeris-toast-min-width` | `CSS custom property` | — | Minimum notification width before viewport constraints. |
| `--aeris-toast-offset` | `CSS custom property` | — | Distance beyond the viewport safe area. |
| `--aeris-toast-mobile-offset` | `CSS custom property` | — | Distance beyond the safe area on narrow viewports. |
...
| `--aeris-toast-severity-tint` | `CSS custom property` | — | Severity color mixed into the notification surface. |
| `--aeris-toast-background` | `CSS custom property` | — | Overrides the severity-tinted notification surface. |
| `--aeris-toast-border-color` | `CSS custom property` | — | Overrides the severity-colored notification border. |
...
Show a plain detail-only notification without an icon or explicit severity.
...
@Component({
  ...
  template: `
    <div>
      ...
      <aeris-toast group="basic" [showIcon]="false" />
    </div>
  `
})
...
Use theme-aware primary and secondary treatments, high-contrast monochrome, or semantic status colors.
...
Stacked mode shows one primary notification and keeps the next visible messages tucked underneath. Hover or focus the stack to smoothly expand the visible window.
...
Drag with a mouse, pen, or touch gesture. The default directions are down and right; short or reversed gestures return smoothly to the resting position.
...
@Component({
  selector: 'app-toast-swipe-demo',
  imports: [AerisButton, AerisToastModule],
  template: `
    <div>
      <div>
        <button aerisButton type="button" (click)="showSwipeToast()">
...
export class ToastSwipeSwipeDismissalDemo {
  private readonly toast = inject(AerisToastService);
...
- Pointer, pen, and touch users can swipe in any configured swipeDirection. Gestures shorter than swipeThreshold return to the resting position, and interactive content does not begin a swipe.
- Use sticky messages for critical information that must remain available until dismissed.
- Entry, dismissal, and stack motion is disabled for users who request reduced motion.
