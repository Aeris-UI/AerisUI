...
class PickListTestHost {
  ...
  target: readonly TestItem[] = [{ id: 'd', label: 'Delta', category: 'Two' }];
...
describe('AerisPickList', () => {
  ...
  it('renders two named multi-select listboxes with draggable options', () => {
    const fixture = setup();
    const listboxes = fixture.nativeElement.querySelectorAll(
      '[role="listbox"]',
    ...
    const itemRadius = getComputedStyle(options.item(0)).borderRadius;
    expect(itemRadius).toContain('--aeris-radius-item');
  });

  it('places reorder controls outside each list viewport', () => {
    const fixture = setup();
    const paneBodies = fixture.nativeElement.querySelectorAll(
      '.aeris-pick-list__pane-body',
    ) as NodeListOf<HTMLElement>;
    const sourceChildren = paneBodies.item(0).children;
    const targetChildren = paneBodies.item(1).children;

    expect(sourceChildren.item(0)?.classList.contains('aeris-pick-list__reorder-controls')).toBe(
      true,
    );
    ...
    expect(targetChildren.item(1)?.classList.contains('aeris-pick-list__reorder-controls')).toBe(
      true,
    );
  });
  ...
  it('filters by multiple configured properties', () => {
    ...
    const sourceOptions = fixture.nativeElement.querySelectorAll(
      '.aeris-pick-list__pane:first-of-type [role="option"]',
    ) as NodeListOf<HTMLElement>;
  ...
  it('transfers items through DOM drag and drop', () => {
    ...
    const targetListbox = fixture.nativeElement.querySelectorAll(
      '[role="listbox"]',
    )[1] as HTMLElement;
