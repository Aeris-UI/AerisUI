...
const document = {
  ...
  library: {
    ...
    principles: [
      ...
      'Aeris components are responsive, themeable, and designed and tested to support WCAG 2.2 Level AA; final conformance depends on the complete application.',
...
function renderLlms(documentation, major) {
  ...
  const lines = [
    ...
    '',
    'Use component secondary entry points and load both global Aeris styles once. Aeris has no required icon library; the documentation uses Lucide only to demonstrate consumer-provided icons. Components are designed and tested to support WCAG 2.2 Level AA, mobile layouts, light and dark themes, RTL, Tailwind CSS, and Bootstrap coexistence; final accessibility conformance depends on the complete application.',
    '',
